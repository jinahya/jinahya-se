...
import com.github.jinahya.lang.JinahyaLangUtils;
 ...
 */
public class JinahyaBeansUtils {
    ...
    private static <T> boolean check(final Class<T> type, final T value) {
        ...
        Objects.requireNonNull(value, "value is null");
        if (type.isPrimitive()
            || JinahyaLangUtils.isWrapper(type)
    ...
    private static <T> boolean checkHelper(final Class<T> type, final Object value) {
        Objects.requireNonNull(type, "type is null");
        if (type.isPrimitive()) {
            return checkHelper(JinahyaLangUtils.getWrapperType(type), value);
        }
    ...
    private static <H extends PropertyInfoHolder, T> void acceptEachPropertyHelper(
            final H parent, final Class<T> clazz, final Object bean,
            final Function<? super H,
                    ? extends Function<? super PropertyInfo,
                            ? extends H>> function)
            throws IntrospectionException, ReflectiveOperationException {
        Objects.requireNonNull(clazz, "clazz is null");
        acceptEachProperty(parent, clazz, clazz.cast(bean), function);
    }
